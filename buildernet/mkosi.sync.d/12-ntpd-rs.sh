...
echo "${EXPECTED_SHA256}" $SRCDIR/buildernet/mkosi.packages/ntpd-rs.deb | sha256sum --check

# postinst script causes errors during mkosi build, remove it
dpkg-deb -R $SRCDIR/buildernet/mkosi.packages/ntpd-rs.deb /tmp/ntpd-rs-deb
rm /tmp/ntpd-rs-deb/DEBIAN/postinst
dpkg -b /tmp/ntpd-rs-deb $SRCDIR/buildernet/mkosi.packages/ntpd-rs.deb
