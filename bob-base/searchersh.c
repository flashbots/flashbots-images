...
int main(int argc, char *argv[]) {
  ...
  if (command == NULL) {
    ...
    fprintf(stderr, "No command provided. Valid commands are: toggle, status, "
                    "logs, tail-the-logs, restart-lighthouse, initialize\n");
  ...
  // If command == "initialize", run the tdx-init program with set-passphrase
  // command
  ...
  if (!is_initialized()) {
    fprintf(stderr,
            "System not initialized. Please run 'initialize' command first.\n");
    free(arg_copy);
    return 1;
  }
  ...
  // If command == "tail-the-logs", print the contents of
  // /persistent/delayed_logs/output.log
  else if (strcmp(command, "tail-the-logs") == 0) {
    execl("/usr/bin/tail", "tail", "-f", "/persistent/delayed_logs/output.log",
          NULL);
  ...
  // If command == "logs", we expect a second token representing number of lines
    ...
    for (int i = 0; arg[i] != '\0'; i++) {
      if (!isdigit((unsigned char)arg[i])) {
        fprintf(stderr,
                "Invalid line count (non-digit characters detected): %s\n",
                arg);
        free(arg_copy);
        return 1;
      }
    ...
    execl("/usr/bin/tail", "tail", "-n", arg,
          "/persistent/delayed_logs/output.log", (char *)NULL);
  ...
  // If command == "restart-lighthouse", restart the lighthouse systemd service
    ...
    execl("/usr/bin/sudo", "sudo", "/usr/bin/systemctl", "restart",
          "lighthouse", NULL);
  ...
  // This command is included in both L1 and L2 bob images, no harm done,
  // but it's way simpler to have common searchersh.c instead of customizing it
  // somehow.
  else if (strcmp(command, "restart-op-node") == 0) {
    execl("/usr/bin/sudo", "sudo", "/usr/bin/systemctl", "restart", "op-node",
          NULL);
  ...
  fprintf(stderr,
          "Invalid command. Valid commands are: toggle, status, logs, "
          "tail-the-logs, restart-lighthouse, restart-op-node, initialize\n");
